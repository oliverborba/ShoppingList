...
class ListTableViewController: UITableViewController {
    ...
    lazy var firestore: Firestore = {
        ...
        let firestote = Firestore.firestore()
        firestote.settings = settings
        return firestote
    }()
    ...
    func loadShoppingList(){
        listener = firestore.collection(collection).order(by: "name", descending: false).addSnapshotListener(includeMetadataChanges: true, listener: { snapshot, error in
            ...
            if let error = error{
                ...
                print("total de documentos alterados: \(snapshot.documents.count)")
    ...
    func showAlertForItem(_ item: ShoppingItem? = nil){
        ...
        let okAction = UIAlertAction(title: "OK", style: .default) { _ in
            ...
            guard let name = alert.textFields?.first?.text, let quantityString = alert.textFields?.last?.text, let quantity = Int(quantityString) else{
                return
            }
